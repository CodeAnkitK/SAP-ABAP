  METHOD generate_eseal_certificate_doc.
*---------------------------------------------------------------------*
*This method is used to fetch the basic details and Authorised Signatory
* for the multiple Intefaces.
*---------------------------------------------------------------------*
* Trigger Point :
*    ZDCL_ODATA_INTERFACE -> TRIGGER_EVENT_FOR_ESEAL
*    ZESEAL_ERR_REN_BG Report
*---------------------------------------------------------------------*
...
*---------------------------------------------------------------------*
    ...
    DATA lv_pdf     TYPE xstring ##NEEDED.
    DATA lv_message TYPE string ##NEEDED.
    DATA lt_corr TYPE isut_umc_correspondence ##NEEDED.
    DATA lt_out TYPE bapiret2_t ##NEEDED.
    DATA lv_flag_err TYPE char1.
    CLEAR lv_flag_err.
    DATA:lv_insert   TYPE boolean VALUE 'X'.
...
*---------------------------------------------------------------------*
    CONSTANTS:
      ...
      lc_delay_cotyp               TYPE char50       VALUE 'ZESEAL_DELAY_COTYP',
      lc_eseal                     TYPE char50       VALUE 'ESEAL',
      lc_delay                     TYPE char50       VALUE 'DELAY',
      lc_x                         TYPE char1        VALUE 'X'.


    "Document Duplcation check.

    SELECT SINGLE cokey, cotyp, gpart, status
      FROM zdc_eseal_log
      INTO @DATA(doc_check)
      WHERE cokey = @iv_fbnum
        AND cotyp = @iv_cotyp
        AND gpart = @iv_taxpayer
        AND status = '03'.

    IF sy-subrc <> 0.

**---------------------------------------------------------------------*
**Creating a certain Delay for certain COTYP if more that one COTYPs
**are getting trigered at the same time.
**---------------------------------------------------------------------*
*      SELECT * FROM tvarvc INTO TABLE @DATA(ls_tvarvc_delay) WHERE name = @lc_delay_cotyp. "#EC CI_SUBRC
*      DATA lv_tvarvc_delay TYPE tvarvc ##NEEDED.
*
*      READ TABLE ls_tvarvc_delay INTO lv_tvarvc_delay WITH KEY low = iv_cotyp.
*      IF sy-subrc EQ 0.
*        SELECT SINGLE value FROM zdc_config INTO @DATA(lv_waittime) WHERE prg_code = @lc_eseal AND param_code = @lc_delay ##WARN_OK.
*        IF sy-subrc EQ 0.
*          WAIT UP TO lv_waittime SECONDS.
*        ENDIF.
*      ENDIF.
*
**---------------------------------------------------------------------*
*      SELECT SINGLE fbsta FROM dfmca_return WHERE fbnum = @iv_fbnum INTO @DATA(lv_fbsta) ##WARN_OK.
*      IF sy-subrc = 0.
*        CASE  lv_fbsta.
*          WHEN 'IP015'.
*            RETURN.
*          WHEN 'IP014'.
*            DATA(lv_process) = abap_true.
*          WHEN 'IP013'.
*            DO 3 TIMES.
*              CLEAR lv_fbsta.
*              SELECT SINGLE fbsta FROM dfmca_return WHERE fbnum = @iv_fbnum INTO @lv_fbsta ##WARN_OK. "#EC CI_SEL_NESTED or "#EC CI_SROFC_NESTED
*              IF sy-subrc = 0.
*                CASE  lv_fbsta.
*                  WHEN 'IP015'.
*                    RETURN.
...
*---------------------------------------------------------------------*
          ...
          DATA: lv_att_id TYPE isu_umc_doc_id.
          DATA: lv_guid TYPE doguid_kk.
          DATA: lv_cokey TYPE cokey_kk.
          CALL METHOD eseal_get_attachment
            EXPORTING
              iv_fbnum         = iv_fbnum
              iv_account_id    = iv_taxpayer
              iv_cotyp         = iv_cotyp
            IMPORTING
              et_correspodance = lt_corr
              ev_data          = lv_guid
              ev_cokey         = lv_cokey.

          lv_att_id = lv_guid.

          IF lv_cokey IS INITIAL .
**---------------------------------------------------------------------*
** Handle Error if CORR structure is Empty.
**---------------------------------------------------------------------*
            CALL METHOD update_log_eseal
              EXPORTING
                is_eseal_log  = ls_eseal_log
          ...
          ELSE.
*---------------------------------------------------------------------*
* CHECK POINT 3:  Check the MEDIA RESOURCE Field id not empty
*                 The program will exit if doesn't aquire the PDF.
*---------------------------------------------------------------------*
            ls_eseal_log-new_cokey = lv_cokey. "++By Monishan on 23-02-24
            IF lv_att_id IS INITIAL .
*---------------------------------------------------------------------*
* Handle Error if attachment is Empty.
*---------------------------------------------------------------------*
              CALL METHOD update_log_eseal
                EXPORTING
                  is_eseal_log  = ls_eseal_log
              ...
              lv_flag_err = lc_x.
            ELSE.
              DATA ls_att TYPE ficas_umc_attachment.
*---------------------------------------------------------------------*
* CHECK POINT 3:  Check the MEDIA RESOURCE Field id not empty
*                 The program will exit if doesn't aquire the PDF.
*---------------------------------------------------------------------*
              CALL FUNCTION 'FICA_UMC_FBATTACHMENT_GET'
                EXPORTING
                  iv_attachmentid = lv_att_id
                IMPORTING
                  es_attachment   = ls_att
                  et_return       = lt_out
*  *       EV_ACCOUNT_ID         =
                .

              IF ls_att-media_resource IS INITIAL.
*---------------------------------------------------------------------*
* Handle Error if no Media Resource Found.
*---------------------------------------------------------------------*
                CALL METHOD update_log_eseal
                  EXPORTING
                    is_eseal_log  = ls_eseal_log
              ...
              ELSE.
                ...
                IF sy-subrc EQ 0.
                  ...
                  ls_eseal_log-request_id = ls_dfkkcoh-cokey.
...
*---------------------------------------------------------------------*
                  ...
                  lv_pdf = ls_att-media_resource.
                  CALL FUNCTION 'SCMS_BASE64_ENCODE_STR'
                    EXPORTING
                      input  = lv_pdf
                    IMPORTING
                      output = ls_request-inputdocumentbase64.
...
*---------------------------------------------------------------------*
* Step 7: Sending request structure to API
...
*---------------------------------------------------------------------*
*CHECK POINT 6: IF lv_pdf is initnal the program will fail
*---------------------------------------------------------------------*
...
*---------------------------------------------------------------------*
                        ...
                        lv_flag_err = lc_x.
                      ELSE.
                        SELECT SINGLE * FROM dfkkdoc INTO CORRESPONDING FIELDS OF ls_dfkkdoc WHERE doguid = ls_dfkkdoc_con-doguid ##WARN_OK. "#EC CI_ALL_FIELDS_NEEDED
                        IF sy-subrc <> 0.
*---------------------------------------------------------------------*
...
*---------------------------------------------------------------------*
              ...
              ENDIF. "Checking on Media Resource
            ENDIF. "Checking on PDF Attachment
          ENDIF. "Checking on LV_COKEY
        ENDIF. "Checking on ESEAL Record.
...
*      ENDIF.
    ...
    ENDIF. "Document Duplcation check.
  ENDMETHOD.
